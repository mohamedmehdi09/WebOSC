...
export async function updateAnnoucementTitle(
  ...
  try {
    ...
    const updateAnnoucementTitleFormParsed =
      updateAnnoucementTitleFormSchema.safeParse(
        updateAnnoucementTitleFormData,
      );

    if (!updateAnnoucementTitleFormParsed.success) {
      throw new ActionError(
        updateAnnoucementTitleFormParsed.error.issues[0].message,
      );
    }

    // check if announcement exists
    const checkAnnouncement = await prisma.announcement.findFirst({
      where: {
        announcement_id: updateAnnoucementTitleFormData.announcement_id,
      },
    });

    if (!checkAnnouncement)
      throw new ActionError("Announcement does not exist!");

    // check if user is editor in announcement org

    const editor = prisma.editor.findFirst({
      where: { org_id: checkAnnouncement.org_id, user_id: user.user_id },
    });
    if (!editor)
    ...
    const updatedAnnouncement = await prisma.announcement.update({
      where: { announcement_id: checkAnnouncement.announcement_id },
      data: { title: updateAnnoucementTitleFormData.title },
    });

    state.success = true;
    state.message = "Announcement title updated successfully!";
  } catch (error) {
    state.success = false;
    if (error instanceof ActionError) state.message = error.message;
